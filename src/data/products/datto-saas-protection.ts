  ...
  sources: [
    {
      ...
      evidenceSummary: "Backups occur 'up to up to three times a day'. Microsoft/Google provide native availability but these are not the same as third-party backups.", evidenceType: "kaseya-product"
    },
    {
      ...
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Infinite Cloud Retention includes daily snapshot thinning, weekly/monthly retention, and configured rules.", evidenceType: "kaseya-product"
    }
  ]
};
...
export const scenarios: Scenario[] = [
  {
    ...
    steps: {
      'step-1': {
        id: 'step-1', sourceRefs: ['src-saas-works'],
      ...
      'step-2': {
        id: 'step-2', sourceRefs: ['src-saas-works'],
      ...
      'step-3': {
        id: 'step-3', sourceRefs: ['src-saas-works'],
...
export const cards: Flashcard[] = [
  { id: 'fc-saas-1', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'How often does Datto SaaS Protection back up M365/Google Workspace by default?', answer: 'up to three times a day automatically.' },
  { id: 'fc-saas-2', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What is the default retention period for Datto SaaS Protection?', answer: 'Typically Infinite Cloud Retention (ICR), meaning data is kept forever as long as the service is active, but 1-year options exist.' },
  { id: 'fc-saas-3', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'Does pausing a user\'s backup delete their historical data?', answer: 'No, pausing (or unprotecting) stops new backups and frees a license seat, but historical data remains per the retention policy.' },
  { id: 'fc-saas-4', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What format are OneDrive/Google Drive files exported in?', answer: 'A standard ZIP file containing the original file formats.' },
  { id: 'fc-saas-5', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What does a "Partial" backup status mean?', answer: 'The backup ran, but some items failed to back up (e.g., due to API throttling from Microsoft/Google or a corrupt item).' },
  { id: 'fc-saas-6', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'Where do direct restores of emails go by default in M365?', answer: 'To a newly created folder named "Datto Restore" with the date and time, to prevent overwriting existing data.' },
  { id: 'fc-saas-7', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'Can Datto SaaS Protection back up Microsoft Teams chats?', answer: 'Yes, it backs up Teams channels, files, and conversations, though API limitations sometimes affect private 1:1 chats depending on configuration.' },
  { id: 'fc-saas-8', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What happens if you turn on "Auto-Add New Users"?', answer: 'SaaS Protection will automatically detect new users with valid licenses in the tenant and begin backing them up without manual intervention.' },
  { id: 'fc-saas-9', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'Is Datto SaaS Protection subject to Microsoft/Google API throttling?', answer: 'Yes. If a tenant is heavily utilized, Microsoft/Google may throttle the API, causing backups to take longer or fail temporarily.' },
  { id: 'fc-saas-10', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'How do you restore a deleted SharePoint document library?', answer: 'Go to the SharePoint section, select the site, pick a snapshot prior to deletion, and select the library to restore.' },
  { id: 'fc-saas-11', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'Can you restore a Google Workspace email to a different user\'s account?', answer: 'Yes, cross-user restores are supported in both Google Workspace and M365.' },
  { id: 'fc-saas-12', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What is a "destructive restore"?', answer: 'A restore that overwrites the existing data in the destination with the data from the backup.' },
  { id: 'fc-saas-13', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'If a user is hard-deleted in M365, what happens to their SaaS Protection backup?', answer: 'The backups are retained indefinitely (if on ICR) even if the user is deleted in M365.' },
  { id: 'fc-saas-14', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What permission level is required to authorize SaaS Protection for an M365 tenant?', answer: 'Global Administrator.' },
  { id: 'fc-saas-15', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'How are exports downloaded?', answer: 'Via the web browser from the Exports tab once the background generation is complete.' },
  { id: 'fc-saas-16', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'What is Point-in-Time Restore?', answer: 'A feature that allows you to roll back an entire mailbox or drive to its exact state at a specific past backup, useful for ransomware.' },
  { id: 'fc-saas-17', sourceRefs: ['src-saas-works'], moduleId: 'datto-saas-protection', question: 'Can you search for specific emails inside a backup without restoring?', answer: 'Yes, you can use the search bar to find emails by subject, sender, or date within a snapshot.' }
];
