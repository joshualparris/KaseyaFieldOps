  ...
  sources: [
    {
      ...
      url: "https://rmm.datto.com/help/en/Content/3NEWUI/Monitors/Monitors.htm",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Alert generation, response actions, and ticket creation.", evidenceType: "kaseya-product"
    },
    {
      id: "src-rmm-devices",
      title: "Datto RMM Device Summary",
      url: "https://rmm.datto.com/help/de/Content/3NEWUI/Devices/DeviceSummary.htm",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Viewing device online status, isolation status, and initiating Web Remote/Agent Browser.", evidenceType: "kaseya-product"
    },
    {
      id: "src-rmm-endpoint-security",
      title: "Datto RMM Endpoint Security",
      url: "https://rmm.datto.com/help/en/Content/3NEWUI/EndpointSecurity.htm",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Ransomware detection, isolation behavior, and access during isolation.", evidenceType: "kaseya-product"
    },
    {
      id: "src-rmm-policies",
      ...
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Policy scope, types, and conflict precedence.", evidenceType: "kaseya-product"
    }
  ]
};
...
export const scenarios: Scenario[] = [
  {
    ...
    steps: {
      'step-1': {
        id: 'step-1', sourceRefs: ['src-rmm-devices'],
      ...
      'step-2': {
        id: 'step-2', sourceRefs: ['src-rmm-devices'],
        ...
        options: [
          {
            id: 'opt-2-1',
            text: 'Check if there is an active Datto EDR alert for the device.', isCorrect: false, feedback: 'While a good instinct, an EDR-isolated device still communicates with Datto RMM (it does not appear offline).',
            nextStepId: 'step-3',
          }
        ]
      },
      'step-3': {
        id: 'step-3', sourceRefs: ['src-rmm-devices'],
        ...
        options: [
          {
            id: 'opt-3-2',
            text: 'Investigated offline status for LAPTOP-014. Verified 6 days offline in RMM. Contacted user jsmith, who confirmed she is on vacation and device is powered off. No further action needed.',
            isCorrect: true,
  ...
  },
  {
    ...
    steps: {
      ...
      'step-2': {
        ...
        options: [
          { id: 'opt-2-1', text: 'Policy conflict and precedence depend on the Datto RMM policy type. You must check the documentation for the specific policy being configured.', isCorrect: true, feedback: 'Exactly. For example, Endpoint Security expects only one policy, while Patch Management supports site-level overrides.', nextStepId: 'step-3' }
        ]
      },
      'step-3': {
        id: 'step-3', sourceRefs: ['src-rmm-devices'],
  ...
  },
  {
    ...
    steps: {
      'step-1': {
        ...
        options: [
          { id: 'opt-1-1', text: 'Use the Agent Browser or Web Remote to connect to the device. Datto RMM management communication and remote control remain permitted according to documented isolation behavior.', isCorrect: true, feedback: 'Correct. While normal LAN/internet communication is restricted by isolation, Datto RMM management communication remains permitted.', nextStepId: 'step-2' }
        ]
      },
      'step-2': {
        id: 'step-2', sourceRefs: ['src-rmm-devices'],
...
export const cards: Flashcard[] = [
  { id: 'fc-rmm-1', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the primary purpose of Datto RMM?', answer: 'Remote Monitoring and Management: managing endpoints, running remote actions, patching, and monitoring device health proactively.' },
  { id: 'fc-rmm-2', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'Where would you look in Datto RMM to see if a device is online?', answer: 'The Device Summary page or the Devices list, looking for the green online indicator next to the hostname.' },
  { id: 'fc-rmm-3', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How is policy conflict and precedence handled in Datto RMM?', answer: 'It depends on the policy type. For example, Endpoint Security expects only one policy, while Patch Management supports site-level overrides. Always check documentation for the specific policy type.' },
  { id: 'fc-rmm-4', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is a "Component" in Datto RMM?', answer: 'A reusable script, application installer, or monitor definition that can be deployed via jobs or policies.' },
  { id: 'fc-rmm-5', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How do you force an immediate check-in for an online device?', answer: 'Use the "Request Device Audit" or "Request Check-in" action from the device summary.' },
  { id: 'fc-rmm-6', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What does "Execution Context" mean in a component?', answer: 'Whether the script runs as the "System" account (admin rights, no user profile) or "Logged In User" (user rights, accesses user profile).' },
  { id: 'fc-rmm-7', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is a "Job" in Datto RMM?', answer: 'A scheduled or immediate task to deploy a component to one or more devices.' },
  { id: 'fc-rmm-8', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is a "Site" in Datto RMM terminology?', answer: 'A logical grouping of devices, typically representing a specific client or a physical location.' },
  { id: 'fc-rmm-9', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How do you deploy third-party software updates?', answer: 'Using the Software Management policies, which handle automated patching for supported third-party apps.' },
  { id: 'fc-rmm-10', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the "Web Remote" feature?', answer: 'The built-in browser-based remote control tool in Datto RMM for screen sharing and command-line access.' },
  { id: 'fc-rmm-11', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is a "Filter" in Datto RMM?', answer: 'A dynamic search query used to group devices based on specific criteria (e.g., OS version, missing patches) for targeting policies or jobs.' },
  { id: 'fc-rmm-12', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'Why might a device show as "Online" but fail to execute a job?', answer: 'The Datto RMM agent service on the endpoint might be hung, or there could be a localized network/firewall issue blocking the job payload.' },
  { id: 'fc-rmm-13', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the purpose of the "Agent Browser"?', answer: 'A technician tool to interact with a device\'s file system, registry, services, and processes in the background without disturbing the user.' },
  { id: 'fc-rmm-14', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How can you automatically resolve an alert when a problem is fixed?', answer: 'Configure the monitor with an "Auto-Resolve" condition (e.g., if CPU drops below 80% for 5 minutes, resolve the alert).' },
  { id: 'fc-rmm-15', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is a "Quick Job"?', answer: 'A way to instantly deploy a single component to selected devices without setting up a full scheduled job.' },
  { id: 'fc-rmm-16', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the policy inheritance hierarchy in Datto RMM for patch management?', answer: 'Site-level patch policies always override Global-level patch policies.' },
  { id: 'fc-rmm-17', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What does "Audit Only" mode do in a Patch Management policy?', answer: 'It scans for and reports on missing patches but explicitly does not install them or force reboots.' },
  { id: 'fc-rmm-18', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What happens to UDF 1 (User-Defined Field 1) when Ransomware Detection isolates a device?', answer: 'Datto RMM automatically populates UDF 1 with the isolation status and timestamp, which can be used to trigger dynamic filters and alerts.' },
  { id: 'fc-rmm-19', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'Can you still remotely access a device that has been network-isolated by Ransomware Detection?', answer: 'Yes, the Datto RMM agent maintains a secure tunnel back to the platform, allowing access via Agent Browser or Web Remote.' },
  { id: 'fc-rmm-20', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How does Datto RMM integrate with Datto BCDR for ransomware recovery?', answer: 'If integrated, RMM can trigger a BCDR restore job directly from the RMM console to recover an isolated device to its last known good backup.' },
  { id: 'fc-rmm-21', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the difference between Standard and Advanced Software Management?', answer: 'Standard covers a few basic apps (like Chrome, Java, Adobe Reader). Advanced covers patching for over 200+ third-party applications.' },
  { id: 'fc-rmm-22', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What runtime framework does the modern Datto RMM Agent and Web Remote use?', answer: 'They moved to .NET 10 to future-proof the agent, as .NET 8 reaches end of support in Nov 2026.' },
  { id: 'fc-rmm-23', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What are the main features of the Agent Browser?', answer: 'It provides background access to the command shell, registry editor, Windows services, event logs, and file system without interrupting the end user.' },
  { id: 'fc-rmm-24', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the purpose of Local Caching in Patch Management?', answer: 'It allows one designated device on a local network to download patches from the internet once, and then distribute them to other local devices to save WAN bandwidth.' },
  { id: 'fc-rmm-25', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How does OS Patching differ from Software Management?', answer: 'OS Patching handles core operating system updates (Windows Update, macOS updates). Software Management handles updates for third-party installed applications.' },
  { id: 'fc-rmm-26', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the ComStore?', answer: 'A built-in repository of pre-written scripts, monitors, and applications created and maintained by Datto for easy deployment.' },
  { id: 'fc-rmm-27', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'Why might a PowerShell script component fail if it prompts for user input?', answer: 'Because scripts deployed via RMM usually run silently in the background (as System). A prompt will hang the execution until it times out.' },
  { id: 'fc-rmm-28', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What are Role-Based Permissions in Datto RMM used for?', answer: 'To restrict technician access, such as preventing tier 1 techs from running destructive scripts or accessing VIP client sites.' },
  { id: 'fc-rmm-29', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is the Executive Summary Report?', answer: 'A scheduled report that provides clients with a high-level overview of their network health, patch compliance, and security posture.' },
  { id: 'fc-rmm-30', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How can you automate the deployment of a specific software package to all newly enrolled devices?', answer: 'Create a Device Filter for new devices missing the software, and assign a recurring Job or an Initial Node configuration policy to that filter.' },
  { id: 'fc-rmm-31', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'What is a "Network Node" in Datto RMM?', answer: 'A specific device designated to perform network discovery scans and act as a local cache or SNMP polling engine for a site.' },
  { id: 'fc-rmm-32', sourceRefs: ['src-rmm-devices'], moduleId: 'datto-rmm', question: 'How do you temporarily suppress alerts for a server during planned maintenance?', answer: 'Place the device in "Maintenance Mode" from the device summary page, which pauses all monitor alerting.' }
];
