  ...
  sources: [
    {
      ...
      evidenceSummary: "K365 User composition: Prevent (INKY, BullPhish, Dark Web ID), Respond (SaaS Alerts), Recover (SaaS Protection or Spanning, Datto Backup for Entra ID).", evidenceType: "kaseya-product"
    },
    {
      id: "src-k365-endpoint-ov",
      title: "Kaseya 365 Endpoint Overview",
      url: "https://help.one.kaseya.com/help/Content/1_Configuration/k365-endpoint-ov.htm",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "K365 Endpoint composition: RMM/VSA, AV, EDR, Advanced Software Management, Endpoint Backup v1/v2, MDR.", evidenceType: "kaseya-product"
    }
  ]
};
...
export const scenarios: Scenario[] = [
  {
    ...
    steps: {
      'step-1': {
        id: 'step-1', sourceRefs: ['src-k365-endpoint-ov'],
      ...
      'step-2': {
        id: 'step-2', sourceRefs: ['src-k365-endpoint-ov'],
  ...
  },
  {
    ...
    steps: {
      ...
      'step-3': {
        id: 'step-3', sourceRefs: ['src-k365-endpoint-ov'],
...
export const cards: Flashcard[] = [
  { id: 'fc-k365-1', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What are the core components of Kaseya 365 Endpoint?', answer: 'Endpoint Management (RMM/VSA), Datto AV, Ransomware detection via Datto EDR, Advanced Software Management, Endpoint Backup v1/v2, and optionally MDR for Endpoint Pro.' },
  { id: 'fc-k365-2', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Why is it dangerous to deploy K365 AV/EDR over an existing third-party AV?', answer: 'It can cause severe system performance degradation, lockups, and kernel panics due to multiple security agents fighting over file access.' },
  { id: 'fc-k365-3', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'How does policy conflict resolution work in Datto RMM?', answer: 'It is policy-type specific. You must check the documentation for the specific policy being configured rather than assuming a universal hierarchy.' },
  { id: 'fc-k365-4', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is a "Baseline" policy?', answer: 'A Global or Site-level policy that establishes the minimum acceptable security or monitoring configuration for all targeted endpoints.' },
  { id: 'fc-k365-5', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'How do you target a script or patch to only a specific subset of machines across multiple clients?', answer: 'Use a Global Device Filter based on specific criteria (e.g., OS version, missing patch), then target a Job to that filter.' },
  { id: 'fc-k365-6', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is the primary function of Endpoint Backup within K365?', answer: 'To provide continuous, file-level or image-level backup for user devices (laptops/desktops) that may frequently be off the corporate network.' },
  { id: 'fc-k365-7', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'If a device is "Isolated" by EDR, how do you access it to remediate the threat?', answer: 'The RMM and EDR agent communication channels remain open during isolation, allowing you to use Web Remote or Agent Browser to connect.' },
  { id: 'fc-k365-8', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What does "Software Management" do in K365?', answer: 'It automates the discovery, approval, and deployment of OS updates and supported third-party application patches.' },
  { id: 'fc-k365-9', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is a "Zero-Day" vulnerability?', answer: 'A software flaw that is actively being exploited before the vendor has released a patch.' },
  { id: 'fc-k365-10', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'How can RMM help during a Zero-Day incident?', answer: 'By using filters to quickly identify vulnerable software versions across the entire fleet and pushing a rapid uninstallation script or emergency patch.' },
  { id: 'fc-k365-11', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is the difference between RMM and EDR?', answer: 'RMM is for IT operations (monitoring health, deploying software, remote control). EDR is for security (detecting malicious behavior, isolating threats).' },
  { id: 'fc-k365-12', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Why should you test patches on a small group before a global rollout?', answer: 'To ensure the patch doesn\'t cause unexpected crashes or break critical line-of-business applications.' },
  { id: 'fc-k365-13', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is the "Agent Browser"?', answer: 'A tool in the RMM that allows technicians to manage services, registry, files, and command line in the background without interrupting the user.' },
  { id: 'fc-k365-14', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is a "Quick Job"?', answer: 'An ad-hoc, immediate deployment of a script or component to a selected group of devices, rather than a recurring scheduled schedule.' },
  { id: 'fc-k365-15', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'How does K365 provide "Unified Visibility"?', answer: 'By integrating RMM, EDR, and Backup status into single dashboard views, reducing the need to switch between multiple portals.' },
  { id: 'fc-k365-16', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Where in Kaseya 365 do you check if a device is protected by all three core security layers (AV, EDR, Backup)?', answer: 'The Kaseya 365 Executive Dashboard or the device\'s unified Security Posture widget.' },
  { id: 'fc-k365-17', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What does a red shield icon typically indicate on the K365 Executive Summary report?', answer: 'A critical failure in one of the security layers, such as the EDR agent being offline or AV definitions being severely out of date.' },
  { id: 'fc-k365-18', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'How can you verify that Endpoint Backup is running successfully across all devices in a tenant?', answer: 'Use the K365 Backup Compliance Report, which aggregates the last successful backup timestamp for all endpoints.' },
  { id: 'fc-k365-19', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'If a user complains of slow performance, how does the K365 dashboard help correlate the issue?', answer: 'You can view CPU/Memory alerts from RMM alongside active EDR scans or Backup jobs running concurrently on the same device.' },
  { id: 'fc-k365-20', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is the "Anti-Virus Compliance" widget designed to show?', answer: 'The percentage of devices that have an active AV agent installed and are running the most recent definition updates.' },
  { id: 'fc-k365-21', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Is INKY Email Security included in the Kaseya 365 User bundle, or is it a separate purchase?', answer: 'INKY is included as the primary email security solution in the Kaseya 365 User bundle.' },
  { id: 'fc-k365-22', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Are BullPhish ID and Dark Web ID included in Kaseya 365 User?', answer: 'Yes, both security awareness training (BullPhish ID) and compromised credential monitoring (Dark Web ID) are included in the User bundle.' },
  { id: 'fc-k365-23', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Is Datto SaaS Protection (M365/Google Workspace backup) included in the Kaseya 365 User bundle?', answer: 'Yes, SaaS Protection/Spanning is a core component of the User bundle for protecting cloud data.' },
  { id: 'fc-k365-24', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'Is SaaS Alerts (cloud application monitoring) included in the Kaseya 365 User bundle?', answer: 'Yes, SaaS Alerts is included to provide monitoring and automated remediation for M365 and Google Workspace environments.' },
  { id: 'fc-k365-25', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'What is the purpose of KaseyaOne?', answer: 'It is the unified single sign-on (SSO) portal and launchpad for accessing all Kaseya modules (like RMM, EDR, INKY, etc.) from one place.' },
  { id: 'fc-k365-26', sourceRefs: ['src-k365-endpoint-ov'], moduleId: 'kaseya-365', question: 'If a module is missing from your KaseyaOne "My Modules" grid, what is the most likely cause?', answer: 'Your user account has not been assigned the necessary role or permissions to access that module in KaseyaOne settings.' }
];
