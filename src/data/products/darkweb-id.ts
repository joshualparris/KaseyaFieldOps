...
export const module: AppModule = {
  ...
  sources: [
    {
      id: "src-dwid-general",
      title: "Dark Web ID Help Center",
      url: "https://help.darkwebid.kaseya.com/help/Content/1_Home/Home.htm",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Domain/email monitoring and PSA ticketing.", evidenceType: "kaseya-product"
    },
    {
      id: "src-dwid-pending",
      title: "NEEDS LIVE-PORTAL / KASEYA TRAINING CONFIRMATION",
      url: "https://help.darkwebid.kaseya.com/pending",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "Botnet Source = data harvested directly via keyloggers needs authoritative confirmation.", evidenceType: "kaseya-product"
    }
  ]
};
...
export const scenarios: Scenario[] = [
  {
    ...
    steps: {
      'step-1': {
        id: 'step-1', sourceRefs: ['src-dwid-general'],
      ...
      'step-2': {
        id: 'step-2', sourceRefs: ['src-dwid-general'],
      ...
      'step-3': {
        id: 'step-3', sourceRefs: ['src-dwid-general'],
...
export const cards: Flashcard[] = [
  { id: 'fc-dwid-1', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What is the primary function of Dark Web ID?', answer: 'To monitor the dark web for compromised credentials, domains, and IP addresses associated with a client.' },
  { id: 'fc-dwid-2', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'Does Dark Web ID prevent breaches?', answer: 'No, it is a detective control. It alerts you AFTER data has been exposed, allowing for rapid response.' },
  { id: 'fc-dwid-3', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What is a "Live Search"?', answer: 'A one-time search used by sales and account managers to find exposures for a prospect without setting up continuous monitoring.' },
  { id: 'fc-dwid-4', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'Why might an exposed password be shown with asterisks (e.g., P@ss****)?', answer: 'Dark Web ID obfuscates passwords by default to protect the user, while showing enough to prove to the user it was their real password.' },
  { id: 'fc-dwid-5', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What does a "Third-Party Breach" mean?', answer: 'The user\'s credentials were stolen from an external service (like LinkedIn or Canva), not directly from the corporate network.' },
  { id: 'fc-dwid-6', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'Why is a third-party breach dangerous to a company?', answer: 'Because users frequently reuse the same password across multiple personal and professional accounts.' },
  { id: 'fc-dwid-7', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'Can Dark Web ID monitor personal email addresses?', answer: 'Yes, you can add specific personal email addresses (like VIPs\' Gmail or Yahoo accounts) to a client\'s watch list.' },
  { id: 'fc-dwid-8', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What does "Botnet" source mean in an alert?', answer: 'The data was likely harvested by malware/keyloggers running directly on an infected device, which is a high-severity indicator.' },
  { id: 'fc-dwid-9', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'How can you reduce the noise of historic breach alerts for a new client?', answer: 'Acknowledge/clear the backlog of old alerts upon initial onboarding so only new, net-new exposures generate active tickets.' },
  { id: 'fc-dwid-10', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'Can Dark Web ID monitor IP addresses?', answer: 'Yes, you can input public static IPs or ranges to see if they are being discussed on hacker forums.' },
  { id: 'fc-dwid-11', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'How do alerts get into the MSP PSA?', answer: 'Via the Integrations section, which can create tickets automatically based on alert severity.' },
  { id: 'fc-dwid-12', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What is a "Combo List"?', answer: 'A compiled list of emails and passwords from various breaches, often used by attackers for credential stuffing.' },
  { id: 'fc-dwid-13', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'If a user has MFA enabled, is an exposed password still a risk?', answer: 'Yes, though the risk is heavily mitigated. The attacker cannot log in directly, but the user should still change the password.' },
  { id: 'fc-dwid-14', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What is the "Dark Web"?', answer: 'A hidden part of the internet requiring specialized software (like Tor) to access, where illicit data is often traded.' },
  { id: 'fc-dwid-15', sourceRefs: ['src-dwid-general'], moduleId: 'darkweb-id', question: 'What should be the immediate technical response to a high-risk credential exposure?', answer: 'Force a password reset for the affected user and terminate their active sessions in AD/M365.' }
];
