  ...
  sources: [
    {
      ...
      evidenceSummary: "Manual IP allowlisting inside INKY is bypassed via integration toggle, but M365 Advanced Delivery must still be configured.", evidenceType: "kaseya-product"
    },
    {
      id: "src-bpid-m365",
      title: "M365 Safelisting Guide",
      url: "https://help.bullphishid.kaseya.com/help/Content/12_Safelisting_guides/Microsoft_Office_365_safelisting_guide.html",
      verifiedAt: "2026-08-20T00:00:00Z",
      evidenceSummary: "M365 Advanced Delivery contains third-party simulation settings.", evidenceType: "kaseya-product"
    }
  ]
};
...
export const scenarios: Scenario[] = [
  {
    ...
    steps: {
      'step-1': {
        id: 'step-1', sourceRefs: ['src-bpid-inky'],
      ...
      'step-2': {
        id: 'step-2', sourceRefs: ['src-bpid-inky'],
      ...
      'step-3': {
        id: 'step-3', sourceRefs: ['src-bpid-inky'],
...
export const cards: Flashcard[] = [
  { id: 'fc-bpid-1', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What is the absolute most critical pre-requisite before running a phishing simulation?', answer: 'Ensuring delivery configuration (e.g., M365 Advanced Delivery) is complete so security tools do not block or falsely "click" the simulations.' },
  { id: 'fc-bpid-2', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What happens to the training data if link scanning is not bypassed?', answer: 'Security tools will automatically scan the links, registering false "clicks" for every user.' },
  { id: 'fc-bpid-3', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'Which Kaseya 365 pillar does BullPhish ID belong to?', answer: 'The Prevent pillar.' },
  { id: 'fc-bpid-4', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'How long does a typical campaign take to set up?', answer: 'About 10 minutes.' },
  { id: 'fc-bpid-5', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What happens if a security tool "clicks" all the links in a test campaign?', answer: 'It ruins the data, resulting in a false 100% click rate. You must fix M365 Advanced Delivery or gateway rules.' },
  { id: 'fc-bpid-6', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What components make up BullPhish ID training?', answer: 'Simulated phishing emails, animated video lessons, and interactive quizzes.' },
  { id: 'fc-bpid-7', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'How does BullPhish ID integrate with INKY?', answer: 'Natively. INKY recognizes the simulations without manual IP allowlisting, and users receive a congratulations banner for correctly reporting.' },
  { id: 'fc-bpid-8', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What is a common compliance use case for BullPhish ID?', answer: 'Providing proof of regular security awareness training for cyber insurance requirements.' },
  { id: 'fc-bpid-9', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'Can you train a multilingual workforce?', answer: 'Yes, campaigns and video content can be localized into multiple languages.' },
  { id: 'fc-bpid-10', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'If a client wants to see who failed the test, where do you look?', answer: 'The Campaign Results or Reporting dashboard.' },
  { id: 'fc-bpid-11', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'Why should you start with generic phishing templates for a new client?', answer: 'To establish a baseline of their security awareness before testing them with highly sophisticated spear-phishing.' },
  { id: 'fc-bpid-12', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What is the purpose of Automated Reminders?', answer: 'To automatically email users who have not yet completed assigned training before the due date.' },
  { id: 'fc-bpid-13', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'Can BullPhish ID simulate internal HR emails?', answer: 'Yes, using custom templates and optionally spoofing the internal domain if DNS records are properly configured.' },
  { id: 'fc-bpid-14', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'What reporting metric is most critical for proving ROI to a client?', answer: 'The reduction in the "Click Rate" and "Submitted Data Rate" over time across multiple campaigns.' },
  { id: 'fc-bpid-15', sourceRefs: ['src-bpid-inky'], moduleId: 'bullphish-id', question: 'If a user opens an email but does not click a link, is it a failure?', answer: 'Opening an email is tracked, but is generally not considered a failure unless they click a link or download an attachment.' }
];
